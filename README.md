...
issue search 把词按 AND 处理，不支持 OR）。

关心的不是模型名而是**推理路径上的组件** —— 上游改 MLA / indexer / DeepEP /
cudagraph 的 PR，标题里往往根本不出现模型名。实测只用模型名会漏掉三成七：
48h 窗口里 `mla` 漏 33 条、`indexer` 漏 22 条，`deepep` 和 `cudagraph` 覆盖率为 0。

15 个词 × PR/issue 两次 = 每轮 28 次 search 请求，而认证后 search 限额正好
30 次/分钟 —— 所以请求间有节流（`SEARCH_GAP_TOKEN`），并对 403/429 读
`Retry-After` 退避重试。不节流会间歇性**部分关键词静默失败**，比整个崩掉更难发现。只匹配标题：实测 `deepseek in:title,body`
...
**三、粗筛评级。** 标题 + 400 字符正文摘要，判 P0/P1/P2 并给一句判级理由。
40 条一批，单批失败只降级那一批。

**四、二次精读。** 粗筛判成 P0/P1 的再读一遍：**6000 字符正文 + labels + 状态**，
产出「这个 PR 做了什么」并允许改判。

为什么需要这一步：上游 PR 正文中位数 4269 字符、p90 7610，而粗筛只喂 400 ——
**99% 被截断**，模型基本只能靠标题推理。实测 vLLM #56969 正文 15136 字符，
粗筛看到的只有模板占位符加一句标题重复。

正文本来就在搜索结果里，所以这一步**不额外消耗 GitHub 配额**，只花模型 token。
但 P0+P1 常占八成以上（实测 304 条里 256 条），全精读等于全量重跑且每条正文是
粗筛的 15 倍，所以有上限 `DEEP_MAX`（默认 80），按 P0 优先、其次今日新增 P1 取。

**旧三、评级。** 交给公司网关的模型评 P0 / P1 / P2，每条给一句中文理由。
40 条一批，单批失败只降级那一批。
...
**卡片只铺「今日新增的 P0」**，存量 P0 折成一行计数 + 各大类跳转链接。
存量是持续风险不是今天的新闻，一起铺会把当天真正的新情况埋掉 ——
实测 P0 里九成以上是存量，卡片体积也从 9KB 降到 2–3KB。
